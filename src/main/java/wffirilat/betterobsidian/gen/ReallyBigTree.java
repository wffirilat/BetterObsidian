...
/**
 * @author Clan Befistrich
 *
 */
public class ReallyBigTree extends WorldGenAbstractTree {
	/**
	...
	public ReallyBigTree(boolean doNotify) {
		super(doNotify);
	}
	...
	void generateLeafNode(int x, int y, int z) {
		...
		for (int i1 = y + this.leafDistanceLimit; h < i1; ++h) {
			float f = this.leafSize(h - y);
			this.func_150529_a(x, h, z, f, (byte) 1, Blocks.leaves);
		}
	}
	...
	void generateLeaves() {
		...
		for (int nodes = this.leafNodes.length; current < nodes; ++current) {
			int x = this.leafNodes[current][0];
			int y = this.leafNodes[current][1];
			int z = this.leafNodes[current][2];
	...
	void generateLeafNodeBases() {
		int node = 0;
		int nodes = this.leafNodes.length;

		for (int[] aint = new int[] { this.basePos[0], this.basePos[1], this.basePos[2] }; node < nodes; ++node) {
			int[] nodePos = this.leafNodes[node];
			int[] nodePos2 = new int[] { nodePos[0], nodePos[1], nodePos[2] };
			aint[1] = nodePos[3];
			int k = aint[1] - this.basePos[1];

			if (this.leafNodeNeedsBase(k)) {
				this.func_150530_a(aint, nodePos2, Blocks.log);
			}
		}
	}
	...
	int checkBlockLine(int[] pos1, int[] pos2) {
		...
		if (aint2[b1] == 0) {
			...
			for (j = aint2[b1] + b4; i != j; i += b4) {
				aint3[b1] = pos1[b1] + i;
				aint3[b2] = MathHelper.floor_double(pos1[b2] + i * d0);
				aint3[b3] = MathHelper.floor_double(pos1[b3] + i * d1);
	...
	boolean validTreeLocation() {
		return true;
		/*
		 * int[] aint = new int[] { this.basePos[0], this.basePos[1],
		 * this.basePos[2] }; int[] aint1 = new int[] { this.basePos[0],
		 * this.basePos[1] + this.heightLimit - 1, this.basePos[2] }; Block
		 * block = this.worldObj.getBlock(this.basePos[0], this.basePos[1] - 1,
		 * this.basePos[2]);
		 * 
		 * boolean isSoil = block.canSustainPlant(worldObj, basePos[0],
		 * basePos[1] - 1, basePos[2], ForgeDirection.UP, (BlockSapling)
		 * Blocks.sapling); if (!isSoil) { return false; } else { int i =
		 * this.checkBlockLine(aint, aint1);
		 * 
		 * if (i == -1) { return true; } else if (i < 6) { return false; } else
		 * { this.heightLimit = i; return true; } }
		 */
	}
	...
	public boolean generate(World p_76484_1_, Random p_76484_2_, int p_76484_3_, int p_76484_4_, int p_76484_5_) {
		...
		if (this.heightLimit == 0) {
			this.heightLimit = this.heightLimitOffset + this.rand.nextInt(this.heightLimitLimit);
		}
