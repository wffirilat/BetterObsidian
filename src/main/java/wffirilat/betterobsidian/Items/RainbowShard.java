...
public class RainbowShard extends Item {
	...
	public boolean generate(World w, Random rand, int x, int y, int z) {
		...
		double x0 = x + 8 + MathHelper.sin(randPI) * number / 8.0F;
		double x1 = x + 8 - MathHelper.sin(randPI) * number / 8.0F;
		double z0 = z + 8 + MathHelper.cos(randPI) * number / 8.0F;
		double z1 = z + 8 - MathHelper.cos(randPI) * number / 8.0F;
		double ry0 = y + rand.nextInt(3) - 2;
		double ry1 = y + rand.nextInt(3) - 2;

		for (int i = 0; i <= this.number; ++i) {
			double x2 = x0 + (x1 - x0) * i / this.number;
			double y2 = ry0 + (ry1 - ry0) * i / this.number;
			double z2 = z0 + (z1 - z0) * i / this.number;
			double r0 = rand.nextDouble() * this.number / 16.0D;
			double d10 = (MathHelper.sin(i * pi / number) + 1.0F) * r0 + 1.0D;
			double d11 = (MathHelper.sin(i * pi / number) + 1.0F) * r0 + 1.0D;
			...
			for (int k2 = i1; k2 <= l1; ++k2) {
				double d12 = (k2 + 0.5D - x2) / (d10 / 2.0D);

				if (d12 * d12 < 1.0D) {
					for (int l2 = j1; l2 <= i2; ++l2) {
						double d13 = (l2 + 0.5D - y2) / (d11 / 2.0D);

						if (d12 * d12 + d13 * d13 < 1.0D) {
							for (int i3 = k1; i3 <= j2; ++i3) {
								double d14 = (i3 + 0.5D - z2) / (d10 / 2.0D);
