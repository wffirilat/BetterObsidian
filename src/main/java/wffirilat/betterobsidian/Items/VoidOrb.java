    				...
    				for(int k1 = -1;k1<=1;k1++){
    					if((int)p.posY+j1>0){
    						if(!p.capabilities.isCreativeMode){
    							p.inventory.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(w.getBlock((int)p.posX+i1, (int)p.posY+j1, (int)p.posZ+k1)),1,
    									w.getBlockMetadata((int)p.posX+i1, (int)p.posY+j1, (int)p.posZ+k1)));
    						}
